// Block type counters for deterministic naming
const blockCounters = {};

function getNextBlockName(type) {
    if (!blockCounters[type]) {
        blockCounters[type] = 0;
    }
    blockCounters[type]++;
    return `${type}_${blockCounters[type]}`;
}

// Generate unique IDs for ports (fallback only)
let idCounter = 0;
function generateId() {
...
class Connector {
    constructor(startPort, endPort) {
        ...
        this.nodes = [];
        this.id = `${startPort.id}_to_${endPort.id}`;
    }
...
class BlockWindow {
    constructor(x, y, width, height, title, type) {
        ...
        this.title = title;
        this.type = type;
        this.blockName = getNextBlockName(type); // Generate unique block name
        this.flipped = false; // For left-right flipping with 'x' key
        this.focused = false;
        this.dragging = false;
        ...
        if (type === 'graph') {
            ...
            this.textContent = 'Text Block';
            this.fontSize = 12;
            this.editingText = false;
        } else if (type === 'nodeOut') {
            this.nodeLabel = 'A';
            this.editingNodeLabel = false;
        } else if (type === 'nodeIn') {
            this.nodeLabel = 'A';
            this.editingNodeLabel = false;
        }
    ...
    setupPorts() {
        ...
        switch (this.type) {
            case 'inputNumber':
            case 'inputSlider':
                this.ports.push(new Port(this.x + this.width, this.y + this.height/2, 'output', `${this.blockName}_output_0`));
                break;
            case 'outputNumber':
                this.ports.push(new Port(this.x, this.y + this.height/2, 'input', `${this.blockName}_input_0`));
                break;
            case 'multiply':
                this.ports.push(new Port(this.x, this.y + this.height/3, 'input', `${this.blockName}_input_0`));
                this.ports.push(new Port(this.x, this.y + 2*this.height/3, 'input', `${this.blockName}_input_1`));
                this.ports.push(new Port(this.x + this.width, this.y + this.height/2, 'output', `${this.blockName}_output_0`));
                break;
            case 'divide':
                this.ports.push(new Port(this.x, this.y + this.height/3, 'input', `${this.blockName}_input_0`));
            ...
            case 'offset':
                ...
                break;
            case 'sineGen':
                this.ports.push(new Port(this.x, this.y + this.height/4, 'input', `${this.blockName}_input_0`)); // Amplitude input
                this.ports.push(new Port(this.x, this.y + this.height/2, 'input', `${this.blockName}_input_1`)); // Frequency input
                this.ports.push(new Port(this.x, this.y + 3*this.height/4, 'input', `${this.blockName}_input_2`)); // Phase input
                this.ports.push(new Port(this.x + this.width, this.y + this.height/2, 'output', `${this.blockName}_output_0`));
                break;
            case 'physicsBasic':
                this.ports.push(new Port(this.x, this.y + this.height/2, 'input', `${this.blockName}_input_0`));
                this.ports.push(new Port(this.x + this.width, this.y + this.height/5, 'output', `${this.blockName}_output_0`));
                this.ports.push(new Port(this.x + this.width, this.y + 2*this.height/5, 'output', `${this.blockName}_output_1`));
                this.ports.push(new Port(this.x + this.width, this.y + 3*this.height/5, 'output', `${this.blockName}_output_2`));
                break;
            case 'springMassDamper':
                ...
                break;
            case 'graph':
                this.ports.push(new Port(this.x, this.y + this.height/4, 'input', `${this.blockName}_input_0`));
                this.ports.push(new Port(this.x, this.y + 3*this.height/4, 'input', `${this.blockName}_input_1`));
                break;
            case 'sineAnalyser':
                // 2 inputs
                this.ports.push(new Port(this.x, this.y + this.height/3, 'input', `${this.blockName}_input_0`)); // Top sine input
                this.ports.push(new Port(this.x, this.y + 2*this.height/3, 'input', `${this.blockName}_input_1`)); // Bottom sine input
                // 5 outputs
                this.ports.push(new Port(this.x + this.width, this.y + this.height/6, 'output', `${this.blockName}_output_0`)); // Top frequency
                this.ports.push(new Port(this.x + this.width, this.y + 2*this.height/6, 'output', `${this.blockName}_output_1`)); // Top amplitude
                this.ports.push(new Port(this.x + this.width, this.y + 3*this.height/6, 'output', `${this.blockName}_output_2`)); // Bottom frequency
                this.ports.push(new Port(this.x + this.width, this.y + 4*this.height/6, 'output', `${this.blockName}_output_3`)); // Bottom amplitude
                this.ports.push(new Port(this.x + this.width, this.y + 5*this.height/6, 'output', `${this.blockName}_output_4`)); // Phase difference
                break;
            case 'clock':
                // 1 output for pulse
                this.ports.push(new Port(this.x + this.width, this.y + this.height/2, 'output', `${this.blockName}_output_0`));
            ...
            case 'counter':
                ...
                this.ports.push(new Port(this.x, this.y + this.height/3, 'input', `${this.blockName}_input_0`)); // Pulse input
                this.ports.push(new Port(this.x, this.y + 2*this.height/3, 'input', `${this.blockName}_input_1`)); // Reset input
                this.ports.push(new Port(this.x + this.width, this.y + this.height/2, 'output', `${this.blockName}_output_0`)); // Count output
                break;
            case 'selector':
                // 2 inputs, 1 output
                this.ports.push(new Port(this.x, this.y + this.height/3, 'input', `${this.blockName}_input_0`)); // Input A
                this.ports.push(new Port(this.x, this.y + 2*this.height/3, 'input', `${this.blockName}_input_1`)); // Input B
                this.ports.push(new Port(this.x + this.width, this.y + this.height/2, 'output', `${this.blockName}_output_0`)); // Selected output
                break;
            case 'xyPlot':
                // 4 inputs: x, y, clk, clear
                this.ports.push(new Port(this.x, this.y + this.height/5, 'input', `${this.blockName}_input_0`)); // X input
                this.ports.push(new Port(this.x, this.y + 2*this.height/5, 'input', `${this.blockName}_input_1`)); // Y input  
                this.ports.push(new Port(this.x, this.y + 3*this.height/5, 'input', `${this.blockName}_input_2`)); // Clock input
                this.ports.push(new Port(this.x, this.y + 4*this.height/5, 'input', `${this.blockName}_input_3`)); // Clear input
                break;
            case 'text':
                // Text blocks don't need ports
                break;
            case 'nodeOut':
                // Input and output for node out block
                this.ports.push(new Port(this.x, this.y + this.height/2, 'input', `${this.blockName}_input_0`)); // Signal input
                this.ports.push(new Port(this.x + this.width, this.y + this.height/2, 'output', `${this.blockName}_output_0`)); // Node output (ball)
                break;
            case 'nodeIn':
                // Only output for node in block (gets signal from matching nodeOut labels)
                this.ports.push(new Port(this.x, this.y + this.height/2, 'output', `${this.blockName}_output_0`)); // Node input (socket) - actually an output
                break;
        }
    ...
    getPortX(isOutput) {
        // When flipped, inputs go to right, outputs go to left
        if (this.flipped) {
            return isOutput ? this.x : this.x + this.width;
        } else {
            return isOutput ? this.x + this.width : this.x;
        }
    }
    ...
    updatePortPositions() {
        for (let i = 0; i < this.ports.length; i++) {
            ...
            switch (this.type) {
                case 'inputNumber':
                case 'inputSlider':
                    port.x = this.getPortX(true); // Output port
                    port.y = this.y + this.height/2;
                    break;
                case 'outputNumber':
                    port.x = this.getPortX(false); // Input port
                    port.y = this.y + this.height/2;
                    break;
                case 'multiply':
                    if (i === 0) {
                ...
                case 'offset':
                    ...
                    break;
                case 'clock':
                    // Clock only has output on the right
                    port.x = this.getPortX(true); // Output port
                    port.y = this.y + this.height/2;
                    break;
                case 'counter':
    ...
    containsSliderArea(x, y, camera, canvas) {
        if (this.type !== 'inputSlider') return false;
        const topLeft = camera.worldToScreen(this.x, this.y, canvas);
        const size = {
            width: this.width * camera.zoom,
            height: this.height * camera.zoom
        };
        const sliderY = topLeft.y + size.height/2;
        const sliderWidth = size.width - 20 * camera.zoom;
        const sliderX = topLeft.x + 10 * camera.zoom;
        
        return x >= sliderX && x <= sliderX + sliderWidth && 
               y >= sliderY - 10 * camera.zoom && y <= sliderY + 10 * camera.zoom;
    }
    
    updateSliderValue(x, camera, canvas) {
        if (this.type !== 'inputSlider') return;
        const topLeft = camera.worldToScreen(this.x, this.y, canvas);
        const size = {
            width: this.width * camera.zoom,
            height: this.height * camera.zoom
        ...
        const relativeX = Math.max(0, Math.min(sliderWidth, x - sliderX));
        const ratio = relativeX / sliderWidth;
    ...
    startSliderParamsEdit() {
        if (this.type === 'inputSlider') {
            this.editingSliderParams = true;
            this.editingSliderField = 'min';
            this.editText = this.min.toString();
            this.editCursor = this.editText.length;
        }
    }
    ...
    startTextEdit() {
        if (this.type === 'text') {
            ...
            this.editText = this.textContent;
            this.editCursor = this.editText.length;
        }
    }
    
    startNodeLabelEdit() {
        if (this.type === 'nodeOut' || this.type === 'nodeIn') {
            this.editingNodeLabel = true;
            this.editText = this.nodeLabel;
            this.editCursor = this.editText.length;
        }
    }
    ...
    handleEditKey(key) {
        if (!this.editing && !this.editingText && !this.editingNodeLabel && !this.editingYLimits && !this.editingSpringParams && !this.editingCounterParams && !this.editingXYLimits && !this.editingSliderParams) return false;
        ...
        // Handle slider parameter editing for inputSlider blocks
        if (this.editingSliderParams) {
            return this.handleSliderParamsEditKey(key);
        }
        ...
        // Handle text editing for text blocks
        if (this.editingText) {
            return this.handleTextEditKey(key);
        }
        
        // Handle node label editing for node blocks
        if (this.editingNodeLabel) {
            return this.handleNodeLabelEditKey(key);
        }
    ...
    handleSliderParamsEditKey(key) {
        if (key === 'Enter') {
            // Save current field and move to next or finish
            this.saveCurrentSliderField();
            this.moveToNextSliderField();
            return true;
        } else if (key === 'ArrowDown') {
            // Save current field and move to next
            this.saveCurrentSliderField();
            ...
            // Save current field and move to previous
            this.saveCurrentSliderField();
            this.moveToPreviousSliderField();
            return true;
        } else if (key === 'Escape') {
            this.editingSliderParams = false;
            this.editingSliderField = null;
            this.editText = '';
            this.editCursor = 0;
            return true;
        } else if (key === 'Backspace') {
    ...
    handleTextEditKey(key) {
        console.log('handleTextEditKey called with key:', key, 'editingText:', this.editingText);
        if (key === 'Enter') {
            // Save text and exit editing
            this.textContent = this.editText;
            this.editingText = false;
            this.editText = '';
            this.editCursor = 0;
            return true;
    ...
    handleNodeLabelEditKey(key) {
        console.log('handleNodeLabelEditKey called with key:', key, 'editingNodeLabel:', this.editingNodeLabel);
        if (key === 'Enter') {
            // Save label and exit editing
            this.nodeLabel = this.editText;
            this.editingNodeLabel = false;
            this.editText = '';
            this.editCursor = 0;
            return true;
    ...
    saveCurrentSliderField() {
        const value = parseFloat(this.editText);
        if (!isNaN(value)) {
            switch (this.editingSliderField) {
                case 'min':
                    this.min = value;
                    break;
                case 'max':
    ...
    moveToNextSliderField() {
        const fields = ['min', 'max', 'value'];
        const currentIndex = fields.indexOf(this.editingSliderField);
        
        if (currentIndex < fields.length - 1) {
            // Move to next field
            this.editingSliderField = fields[currentIndex + 1];
            const currentValue = this.getCurrentSliderFieldValue();
            this.editText = currentValue.toString();
            this.editCursor = this.editText.length;
        } else {
            // Finished editing all fields
    ...
    update(dt, inputs) {
        switch (this.type) {
            ...
            case 'counter':
                ...
                // Initialize frame-based edge detection flags if not present
                if (this.frameResetEdgeDetected === undefined) this.frameResetEdgeDetected = false;
                if (this.framePulseEdgeDetected === undefined) this.framePulseEdgeDetected = false;
                if (this.lastFrameTime === undefined) this.lastFrameTime = 0;
                
                // Reset edge detection flags at start of new frame
                const currentTime = Date.now();
                if (currentTime !== this.lastFrameTime) {
                    this.frameResetEdgeDetected = false;
                    this.framePulseEdgeDetected = false;
                    this.lastFrameTime = currentTime;
                }
                
                // Reset edge detection - reset when input goes from 0 to 1 (once per frame)
                if (!this.frameResetEdgeDetected && resetInput > 0.5 && this.lastResetInput <= 0.5) {
                    this.count = this.startValue;
                    this.frameResetEdgeDetected = true;
                }
                
                // Pulse edge detection - count when input goes from 0 to 1 (once per frame)
                if (!this.framePulseEdgeDetected && pulseInput > 0.5 && this.lastInput <= 0.5) {
                    this.count += this.stepSize;
                    this.framePulseEdgeDetected = true;
            ...
            case 'xyPlot':
                ...
                break;
            case 'nodeOut':
                // NodeOut simply passes input to output
                const nodeOutInput = inputs[this.ports[0].id] || 0;
                this.ports[1].value = nodeOutInput; // Output the input value
                break;
            case 'nodeIn':
                // NodeIn gets its value from virtual connections to matching nodeOut blocks
                // This will be handled in the connection update system
                // The port value is set by the virtual connection system in updateConnections()
    ...
    draw(ctx, camera, canvas, colors) {
        ...
        if (this.type === 'gain') {
            ...
            // Draw normal rectangle for other blocks (except text blocks)
        ...
        if (this.type !== 'text' && this.type !== 'nodeOut' && this.type !== 'nodeIn' && this.type !== 'inputSlider' && this.type !== 'outputNumber') {
            ctx.fillText(this.title, topLeft.x + size.width/2, topLeft.y + 15 * camera.zoom);
        }
        
        // Show title for inputSlider only if it's not empty
        if (this.type === 'inputSlider' && this.title && this.title.trim() !== '') {
            ctx.fillText(this.title, topLeft.x + size.width/2, topLeft.y + 10 * camera.zoom);
        }
        
        // Show title for outputNumber only if it's not empty
        if (this.type === 'outputNumber' && this.title && this.title.trim() !== '') {
            ctx.fillText(this.title, topLeft.x + size.width/2, topLeft.y + 10 * camera.zoom);
        }
        
        if (this.type === 'inputSlider') {
            // Position slider in center of the block
            const sliderY = topLeft.y + size.height/2;
            const sliderWidth = size.width - 20 * camera.zoom;
            const sliderX = topLeft.x + 10 * camera.zoom;
            
            // Draw slider track
            ctx.strokeStyle = this.isDraggingSlider ? themeColors.secondary : themeColors.primary;
            ...
            ctx.fillStyle = this.isDraggingSlider ? themeColors.secondary : themeColors.primary;
            ctx.beginPath();
            ctx.arc(knobX, sliderY, 8 * camera.zoom, 0, Math.PI * 2);
            ctx.fill();
            
            // Value is shown in parameter editing mode or can be seen via connections
        }
        
        // Show slider parameter editing interface if editing
        if (this.editingSliderParams) {
            ctx.fillStyle = themeColors.background;
            ctx.fillRect(topLeft.x + 5, topLeft.y + 5, size.width - 10, 45 * camera.zoom);
            ctx.strokeStyle = themeColors.primary;
            ctx.strokeRect(topLeft.x + 5, topLeft.y + 5, size.width - 10, 45 * camera.zoom);
            ...
            const fields = [
                { label: 'Min:', value: this.min, key: 'min' },
                { label: 'Max:', value: this.max, key: 'max' },
                { label: 'Value:', value: this.value, key: 'value' }
            ];
            ...
            for (let i = 0; i < fields.length; i++) {
                ...
                if (this.editingSliderField === field.key) {
                    // Highlight current editing field
                    ctx.fillStyle = themeColors.primary;
                    ctx.fillRect(x, y - 6 * camera.zoom, 80 * camera.zoom, 10 * camera.zoom);
                    ctx.fillStyle = themeColors.background;
                    ...
                    ctx.lineWidth = 1;
                    ctx.beginPath();
                    ctx.moveTo(x + 2 + textWidth, y - 4 * camera.zoom);
                    ctx.lineTo(x + 2 + textWidth, y + 2 * camera.zoom);
                    ctx.stroke();
                } else {
                    // Normal field display
                    ctx.fillStyle = themeColors.primary;
                    const roundedValue = Math.round(field.value * 100) / 100;
                    ctx.fillText(`${field.label} ${roundedValue}`, x + 2, y);
                }
            }
            ...
            ctx.textAlign = 'center';
            ctx.fillStyle = themeColors.primary;
            ctx.fillText('↓/↑: navigate fields, Enter: save, Esc: cancel', topLeft.x + size.width/2, topLeft.y + 42 * camera.zoom);
        }
        ...
        } else if (this.type === 'outputNumber') {
            // Center the value in the compact output block
            ctx.fillText(this.value.toFixed(2), topLeft.x + size.width/2, topLeft.y + size.height/2 + 4 * camera.zoom);
        }
        ...
        if (this.type === 'text') {
            // Text block rendering - no border, just text
            ctx.font = `${this.fontSize * camera.zoom}px Courier New`;
            ctx.fillStyle = themeColors.primary;
            ctx.textAlign = 'left';
            ...
            if (this.editingText) {
                ...
                ctx.fillStyle = themeColors.background;
                ctx.fillRect(topLeft.x + 2, topLeft.y + 2, size.width - 4, size.height - 4);
                ctx.strokeStyle = themeColors.primary;
                ctx.lineWidth = 1;
                ctx.strokeRect(topLeft.x + 2, topLeft.y + 2, size.width - 4, size.height - 4);
                
                ctx.fillStyle = themeColors.primary;
                ctx.fillText(this.editText, topLeft.x + 5 * camera.zoom, topLeft.y + 25 * camera.zoom);
                
                // Draw cursor
                const textWidth = ctx.measureText(this.editText.substring(0, this.editCursor)).width;
                ctx.fillRect(topLeft.x + 5 * camera.zoom + textWidth, topLeft.y + 10 * camera.zoom, 1, 15 * camera.zoom);
            } else {
                // Show text content - no border
                ctx.fillText(this.textContent, topLeft.x + 5 * camera.zoom, topLeft.y + 25 * camera.zoom);
            }
            
            ctx.textAlign = 'center';
        }
        
        if (this.type === 'nodeOut') {
            // Draw nodeOut block with ball-shaped output port and label
            ctx.font = `${10 * camera.zoom}px Courier New`;
            ctx.fillStyle = themeColors.primary;
            ctx.textAlign = 'left';
            ...
            if (this.editingNodeLabel) {
                ...
                ctx.fillStyle = themeColors.primary;
                ctx.fillText(this.editText, topLeft.x + 5 * camera.zoom, topLeft.y + 20 * camera.zoom);
                
                // Draw cursor
                const textWidth = ctx.measureText(this.editText.substring(0, this.editCursor)).width;
                ctx.fillRect(topLeft.x + 5 * camera.zoom + textWidth, topLeft.y + 10 * camera.zoom, 1, 12 * camera.zoom);
            } else {
                // Show label
                ctx.fillText(this.nodeLabel, topLeft.x + 15 * camera.zoom, topLeft.y + 20 * camera.zoom);
            }
            
            // Draw ball-shaped output port (override normal port drawing)
            const outputPort = this.ports.find(p => p.type === 'output');
            const ballX = outputPort ? outputPort.x : topLeft.x + size.width;
            const ballY = outputPort ? outputPort.y : topLeft.y + size.height/2;
            const ballRadius = 6 * camera.zoom;
        ...
        if (this.type === 'nodeIn') {
            // Draw nodeIn block with socket-shaped input port and label
            ctx.font = `${10 * camera.zoom}px Courier New`;
            ctx.fillStyle = themeColors.primary;
            ctx.textAlign = 'left';
            ...
            // Draw socket-shaped input port (override normal port drawing)
            const inputPort = this.ports.find(p => p.type === 'output'); // nodeIn has output port
            const socketX = inputPort ? inputPort.x : topLeft.x;
            const socketY = inputPort ? inputPort.y : topLeft.y + size.height/2;
            const socketRadius = 6 * camera.zoom;
        ...
        // Skip normal port drawing for node blocks (they have custom port shapes)
        if (this.type !== 'nodeOut' && this.type !== 'nodeIn') {
...
class CommandWindow {
    constructor(x, y) {
        ...
        this.commands = [
            ...
            'multiply', 'divide', 'gain', 'offset', '20log10', '10log10', 'deg2rad', 'rad2deg', 'sineGen', 'sineAnalyser', 'clock', 'counter', 'selector', 'massOnPlane', 'springMassDamper', 'graph', 'grid', 'a2g', 'delete', 'save', 'saveall', 'load', 'disconnect', 'clearAll', 'text', 'nodeOut', 'nodeIn', 'lockMove', 'lockSpace', 'lockZoom', 'lockPan', 'lockConnectors', 'lockFocus', 'lockSpaceAndSave', 'xyPlot'
        ];
        this.completions = [];
...
class DraggableWindows {
    constructor(canvas) {
        ...
        this.lockZoom = false;
        this.lockPan = false;
        this.lockConnectors = false;
        this.lockFocus = false;
        this.theme = this.getCurrentTheme();
        this.isRunning = true; // Run/stop state for performance control
    ...
    isRunStopButtonClick(x, y) {
        ...
        const buttonY = this.canvas.height - buttonHeight - margin;
    ...
    toggleRunStop() {
        ...
        console.log(`[${this.canvas.id}] ${this.isRunning ? 'Running' : 'Stopped'}`);
    }
    ...
    drawRunStopButton() {
        ...
        this.ctx.fillStyle = this.isRunning ? colors.primary : colors.secondary;
        ...
        // Draw button text
        this.ctx.fillStyle = this.isRunning ? colors.background : colors.primary;
    ...
    handleMouseDown(e) {
        ...
        if (this.isRunStopButtonClick(x, y)) {
            this.toggleRunStop();
            return;
        }
            ...
            for (let port of window.ports) {
                if (port.contains(x, y, this.camera, this.canvas)) {
                    if (port.type === 'input') {
                        ...
                        if (!this.lockConnectors && port.connections.length > 0) {
        ...
        for (let i = this.windows.length - 1; i >= 0; i--) {
            ...
            if (window.containsSliderArea && window.containsSliderArea(x, y, this.camera, this.canvas)) {
                this.stopAllEditing();
                this.focusWindow(window);
                window.isDraggingSlider = true;
                window.updateSliderValue(x, this.camera, this.canvas);
                return;
            }
            if (window.contains(x, y, this.camera, this.canvas)) {
                this.stopAllEditing();
                ...
                if (this.lastClickWindow === window && currentTime - this.lastClickTime < 300) {
                    ...
                    } else if (window.type === 'inputSlider') {
                        // Start slider parameter editing
                        window.startSliderParamsEdit();
                    } else if (window.type === 'text') {
                        // Start text editing
                        this.focusWindow(window);
                        window.startTextEdit();
                    } else if (window.type === 'nodeOut' || window.type === 'nodeIn') {
                        // Start node label editing
                        this.focusWindow(window);
                        window.startNodeLabelEdit();
                    } else {
                        // Single click - start dragging (only if movement not locked)
                        if (!this.lockMove) {
                            window.startDrag(x, y, this.camera, this.canvas);
                        } else {
                            console.log(`[${this.canvas.id}] Movement is locked`);
                        }
                    }
                } else {
                    // Single click - start dragging (only if movement not locked)
                ...
                for (let window of this.windows) {
                    for (let port of window.ports) {
                        if (port.contains(x, y, this.camera, this.canvas) && 
                            ...
                            port.type === 'input' && !this.lockConnectors) {
    ...
    handleKeyDown(e) {
        ...
        for (let window of this.windows) {
            if ((window.editing || window.editingText || window.editingNodeLabel || window.editingYLimits || window.editingSpringParams || window.editingCounterParams || window.editingXYLimits || window.editingSliderParams) && window.handleEditKey(e.key)) {
        ...
        if (e.key === ' ' || e.code === 'Space' || e.key === 'Spacebar') {
            ...
            // Check if space key is locked
            if (this.lockSpace) {
                console.log(`[${this.canvas.id}] Space key is locked, command menu blocked`);
                return false;
            }
        ...
        // 'x' key flips blocks when hovering
        if (e.key === 'x' || e.key === 'X') {
            // Find block under mouse cursor
            for (let window of this.windows) {
                if (window.contains(this.currentMousePos.x, this.currentMousePos.y, this.camera, this.canvas)) {
                    window.flipped = !window.flipped;
                    window.updatePortPositions(); // Update port positions after flipping
                    console.log(`[${this.canvas.id}] Flipped ${window.title} block`);
                    e.preventDefault();
                    return;
                }
            }
    ...
    stopAllEditing() {
        for (let window of this.windows) {
            ...
            if (window.editingSliderParams) {
                window.editingSliderParams = false;
                window.editingSliderField = null;
                window.editText = '';
                window.editCursor = 0;
            }
        }
    ...
    executeCommand(command) {
        ...
        switch (command.toLowerCase()) {
            case 'innum':
                ...
                break;
            case 'inslider':
                const slider = this.createWindow(worldPos.x - 62.5, worldPos.y + 50, 125, 25, '', 'inputSlider');
                slider.min = -10;
                slider.max = 10;
                slider.value = 0;
                break;
            case 'outnum':
                this.createWindow(worldPos.x - 37.5, worldPos.y + 50, 75, 25, '', 'outputNumber');
                break;
            case 'multiply':
            ...
            case 'disconnect':
                ...
                console.log(`[${this.canvas.id}] All connections removed`);
                break;
            case 'clearall':
                this.clearAll();
                console.log(`[${this.canvas.id}] Canvas cleared - all blocks and connections removed`);
                break;
            case 'save':
                ...
                break;
            case 'saveall':
                console.log(`[${this.canvas.id}] Save command executed with graph data on system with`, this.windows.length, 'windows');
                this.saveState(true);
                break;
            case 'load':
                this.loadState();
                break;
            case 'text':
                this.createWindow(worldPos.x - 75, worldPos.y - 25, 150, 50, 'Text', 'text');
                break;
            case 'nodeout':
                this.createWindow(worldPos.x - 40, worldPos.y - 15, 80, 30, 'NodeOut', 'nodeOut');
                break;
            case 'nodein':
                this.createWindow(worldPos.x - 40, worldPos.y - 15, 80, 30, 'NodeIn', 'nodeIn');
                break;
            case 'lockmove':
                this.lockMove = !this.lockMove;
                console.log(`[${this.canvas.id}] Movement lock ${this.lockMove ? 'enabled' : 'disabled'}`);
                break;
            case 'lockspace':
                this.lockSpace = !this.lockSpace;
                console.log(`[${this.canvas.id}] Space menu lock ${this.lockSpace ? 'enabled' : 'disabled'}`);
                break;
            case 'lockzoom':
                this.lockZoom = !this.lockZoom;
                console.log(`[${this.canvas.id}] Zoom lock ${this.lockZoom ? 'enabled' : 'disabled'}`);
                break;
            case 'lockpan':
                this.lockPan = !this.lockPan;
                console.log(`[${this.canvas.id}] Pan lock ${this.lockPan ? 'enabled' : 'disabled'}`);
                break;
            case 'lockconnectors':
                this.lockConnectors = !this.lockConnectors;
                console.log(`[${this.canvas.id}] Connector lock ${this.lockConnectors ? 'enabled' : 'disabled'}`);
                break;
            case 'lockfocus':
                this.lockFocus = !this.lockFocus;
                console.log(`[${this.canvas.id}] Focus lock ${this.lockFocus ? 'enabled' : 'disabled'}`);
                break;
            case 'lockspaceandsave':
                this.lockSpace = true;
                console.log(`[${this.canvas.id}] Space menu locked and saving state`);
                this.saveState(false, true); // false for includeGraphData, true for isSpacebarLocked
                break;
            default:
    ...
    removeAllConnections() {
        console.log(`[${this.canvas.id}] Removing ${this.connectors.length} connections`);
        ...
        console.log(`[${this.canvas.id}] All connections removed successfully`);
    }
    
    clearAll() {
        console.log(`[${this.canvas.id}] Clearing canvas - removing ${this.windows.length} windows and ${this.connectors.length} connections`);
        
        // Stop all editing modes first
        this.stopAllEditing();
        ...
        // Reset block counters for deterministic naming
        for (let key in blockCounters) {
            blockCounters[key] = 0;
        }
        ...
        console.log(`[${this.canvas.id}] Canvas cleared successfully`);
    }
    ...
    focusWindow(window) {
        ...
        // Only reorder windows if focus is not locked
        if (!this.lockFocus) {
    ...
    updateConnections() {
        ...
        // Create virtual connections between nodeOut and nodeIn blocks with matching labels
        const nodeOutBlocks = this.windows.filter(w => w.type === 'nodeOut');
        const nodeInBlocks = this.windows.filter(w => w.type === 'nodeIn');
        
        
        for (let nodeOut of nodeOutBlocks) {
            for (let nodeIn of nodeInBlocks) {
                if (nodeOut.nodeLabel === nodeIn.nodeLabel) {
                    // Virtual connection: nodeIn output gets value from nodeOut output
                    const nodeOutValue = nodeOut.ports[1].value; // nodeOut output port
                    nodeIn.ports[0].value = nodeOutValue; // Set the port value directly
                    values[nodeIn.ports[0].id] = nodeOutValue; // Also set in values map
                }
            }
        }
    ...
    saveState(includeGraphData = false, isSpacebarLocked = false) {
        ...
        console.log('Include graph data:', includeGraphData);
        console.log('Spacebar locked:', isSpacebarLocked);
        ...
        const state = {
            ...
            windows: this.windows.map(w => {
                const windowData = {
                    ...
                    graphData1: includeGraphData ? (w.graphData1 || []) : [],
                    graphData2: includeGraphData ? (w.graphData2 || []) : [],
                    ...
                    // XY Plot properties
                    plotData: includeGraphData ? (w.plotData || []) : [],
                    xMin: w.xMin,
                    xMax: w.xMax,
                    yMin: w.yMin,
                    yMax: w.yMax,
                    ...
                    // Counter block properties
                    startValue: w.startValue,
                    stopValue: w.stopValue,
                    stepSize: w.stepSize,
                    count: w.count,
                    lastInput: w.lastInput,
                    lastResetInput: w.lastResetInput,
                    // Text block properties
                    textContent: w.textContent,
                    fontSize: w.fontSize,
                    editingText: w.editingText,
                    // Node block properties
                    nodeLabel: w.nodeLabel,
                    editingNodeLabel: w.editingNodeLabel
                };
            ...
            lockStates: {
                ...
                lockPan: this.lockPan,
                lockConnectors: this.lockConnectors,
                lockFocus: this.lockFocus
            }
        };
        ...
        const spacebarLockedSuffix = isSpacebarLocked ? '-SpacebarLocked' : '';
        const filename = `simulation-${timestamp}${spacebarLockedSuffix}.json`;
    ...
    async loadDemo() {
        console.log(`[${this.canvas.id}] Loading demo`);
        try {
            // Try to load from EvB23.json first
            const response = await fetch('EvB23.json');
            if (response.ok) {
                const state = await response.json();
                console.log(`[${this.canvas.id}] Loaded EvB23.json successfully`);
                this.restoreState(state);
                return;
            }
        } catch (error) {
            console.log(`[${this.canvas.id}] Could not load EvB23.json, using fallback demo`);
        }
        
        // Fallback to sine analyzer demo
        this.populateSineAnalyzerDemo();
    }
    ...
    restoreState(state) {
        ...
        // Restore block counters to maintain proper naming
        if (state.blockCounters) {
            Object.assign(blockCounters, state.blockCounters);
        } else {
            // Clear counters for old save files without blockCounters
            Object.keys(blockCounters).forEach(key => delete blockCounters[key]);
        }
        ...
        for (let windowData of state.windows) {
            ...
            // Restore blockName if available
            if (windowData.blockName) {
                window.blockName = windowData.blockName;
                // Regenerate ports with correct IDs
                window.setupPorts();
                
                // Restore port data AFTER setupPorts() to ensure correct IDs
                if (windowData.ports) {
                    for (let i = 0; i < window.ports.length && i < windowData.ports.length; i++) {
                        ...
                        window.ports[i].x = windowData.ports[i].x;
                        window.ports[i].y = windowData.ports[i].y;
                    }
                }
            }
            
            // Restore flipped state
            if (windowData.flipped !== undefined) {
                window.flipped = windowData.flipped;
                window.updatePortPositions(); // Update port positions for flipped state
            }
            ...
            // Restore counter block properties
            if (windowData.startValue !== undefined) {
                window.startValue = windowData.startValue;
            }
            if (windowData.stopValue !== undefined) {
                window.stopValue = windowData.stopValue;
            }
            if (windowData.stepSize !== undefined) {
                window.stepSize = windowData.stepSize;
            }
            if (windowData.count !== undefined) {
                window.count = windowData.count;
            }
            if (windowData.lastInput !== undefined) {
                window.lastInput = windowData.lastInput;
            }
            if (windowData.lastResetInput !== undefined) {
                window.lastResetInput = windowData.lastResetInput;
            }
            
            // Restore text block properties
            if (windowData.textContent !== undefined) {
                window.textContent = windowData.textContent;
            }
            if (windowData.fontSize !== undefined) {
                window.fontSize = windowData.fontSize;
            }
            if (windowData.editingText !== undefined) {
                window.editingText = windowData.editingText;
            }
            
            // Restore node block properties
            if (windowData.nodeLabel !== undefined) {
                window.nodeLabel = windowData.nodeLabel;
            }
            if (windowData.editingNodeLabel !== undefined) {
                window.editingNodeLabel = windowData.editingNodeLabel;
            }
            
            // Restore port data for blocks without blockName (fallback)
            if (!windowData.blockName && windowData.ports) {
                for (let i = 0; i < window.ports.length && i < windowData.ports.length; i++) {
                    window.ports[i].id = windowData.ports[i].id;
                    window.ports[i].value = windowData.ports[i].value || 0;
                    window.ports[i].x = windowData.ports[i].x;
        ...
        // Restore lock states if they exist in the saved state
        if (state.lockStates) {
            this.lockMove = state.lockStates.lockMove || false;
            this.lockSpace = state.lockStates.lockSpace || false;
            this.lockZoom = state.lockStates.lockZoom || false;
            this.lockPan = state.lockStates.lockPan || false;
            this.lockConnectors = state.lockStates.lockConnectors || false;
            this.lockFocus = state.lockStates.lockFocus || false;
            console.log('Lock states restored:', state.lockStates);
        }
    }
    ...
    animate() {
        ...
        if (this.isRunning) {
            const dt = 1/60;
            let inputs = this.updateConnections();
            
            // First pass: update all windows
            for (let window of this.windows) {
                window.update(dt, inputs);
            }
            
            // Second pass: update connections again to propagate new output values
            // This ensures that blocks that generate outputs (like counters) make their
            // new values available to other blocks (like xyPlot) in the same frame
            inputs = this.updateConnections();
            
            // Third pass: update only blocks that consume the updated values
            // Focus on blocks that do edge detection and need immediate updates
            for (let window of this.windows) {
                if (window.type === 'xyPlot') {
                    window.update(dt, inputs);
                }
            }
        } else {
            // When stopped, still update connections to show current values
        ...
        // Draw timestamp overlay in top-left corner (unaffected by camera)
        this.ctx.save();
        ...
        this.ctx.fillText(timestamp, 10, 20);
        this.ctx.restore();
        
        // Draw run/stop button (unaffected by camera)
        this.ctx.save();
        this.ctx.setTransform(1, 0, 0, 1, 0, 0); // Reset to screen coordinates
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (canvas0) {
        console.log('Creating system0 for canvas0');
        const system0 = new DraggableWindows(canvas0);
        
        console.log('Loading MassOnPlane demo for system0');
        // Load the embedded MassOnPlane demo data
        if (window.MASS_ON_PLANE_DEMO) {
            system0.restoreState(window.MASS_ON_PLANE_DEMO);
            console.log('MassOnPlane demo loaded successfully');
        } else {
            console.error('MassOnPlane demo data not found');
        }
    }
    ...
    if (canvas2) {
        ...
        console.log('Loading EvB24 demo for system2');
        // Load the embedded EvB24 demo data
        if (window.EVB24_DEMO) {
            system2.restoreState(window.EVB24_DEMO);
            console.log('EvB24 demo loaded successfully');
        } else {
            console.error('EvB24 demo data not found');
            // Fallback to old embedded demo
            system2.loadDemo();
        }
    }
